...
class RoboschoolInvertedPendulum2(RoboschoolMujocoXmlEnv):
    '''
    Continuous control version of classic cartpole problem.
    Keep a pole upright by moving the 1-D cart.
    Similar to MuJoCo InvertedPendulum task. Has an optional version
    ...
    @staticmethod
    def state_argument(state):
        return np.array([
            state[0], state[1],
            np.cos(state[2]), np.sin(state[2]), state[3]
        ])
...
class RoboschoolInvertedPendulumSwingup2(RoboschoolInvertedPendulum):
    swingup = True
...
class RoboschoolInvertedPendulum_plot(RoboschoolMujocoXmlEnv):
    '''
    Continuous control version of classic cartpole problem.
    Keep a pole upright by moving the 1-D cart.
    Similar to MuJoCo InvertedPendulum task. Has an optional version
    ...
    def step(self, a):
        self.apply_action(a)
        self.scene.global_step()
        th, th_dot = self.j1.current_position()
        x, vx = self.slider.current_position()
        ...
        self.reward += sum(self.rewards)
        self.HUD(state, a, done)
        return state, np.array([x, vx, th, th_dot]), sum(self.rewards), done, {}

    def camera_adjust(self):
        self.camera.move_and_look_at(0,1.2,1.0, 0,0,0.5)

class RoboschoolInvertedPendulumSwingup_plot(RoboschoolInvertedPendulum_plot):
    swingup = True