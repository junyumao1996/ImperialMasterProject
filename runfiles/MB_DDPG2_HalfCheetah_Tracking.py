...
import time
from envs.modified_cheetah import HalfCheetahTrackingEnv, HalfCheetahEnv
...
learned_model = Model_Learner(env,
                              ...
                              num_hidden_layers_r=3,
                              num_hidden_units_d=200)
# print('Pre-Train Models ...')
# learned_model.rollouts_collection(learned_model.env, learned_model.experience_dataset, num_rollouts=3, len_rollouts=300)
...
def reward_function(state,action):
    return -0.5 * (state[0] - 10)**2 - 0.2 * np.square(action).sum()
...
while iteration_counter < args.num_episodes:
    ...
    if var >= 0.1 and (ep_type_number % 4 == 0):
    ...
    if ep_type_number == 0:
        ...
        for j in range(args.num_steps):
            ...
            if j in frozen_start_indices:
                ...
                r_eval = reward_function(s_next_eval, a_eval)
        ...
        if training_flag and iteration_counter < 150:
